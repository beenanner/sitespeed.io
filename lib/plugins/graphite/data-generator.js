...
function keyPathFromMessage(message, options, includeQueryParams) {
  ...
  if (message.url) {
    const urlObject = find(options.urls, {'url': message.url})
    if(urlObject && urlObject.alias){
       typeParts.splice(1, 0, flatten.keypathFromAlias(message.url, urlObject.alias));
    } else {
