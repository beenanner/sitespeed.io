...
module.exports = {
  get(dataCollection, options, errors) {
    ...
    let summaryUrl = options.url[0];
    if (options.urls[0].url) {
      summaryUrl = options.urls[0].url;
    }
    let summaryText = `${h.plural(Object.keys(dataCollection.urlPages).length,'page')} analyzed for ` + summaryUrl +
